...
public class StreamingServlet extends HttpServlet {
            ...
            throws IOException
    {
        ...
        try {
            baseFileName = request.getPathInfo().substring(1);
        } catch (NumberFormatException ex) {
            response.sendError(HttpServletResponse.SC_NOT_FOUND, "Invalid podcast base name: " + request.getPathInfo());
            return;
        }

        Episode episode = service.findEpisode(baseFileName);
        if (null == episode) {
            response.sendError(HttpServletResponse.SC_NOT_FOUND, "Podcast base name not found: " + baseFileName);
            return;
        }

        String requestedFile = episode.getFileName();
        File file = new File(episode.getFileName());

        // Check if file actually exists in filesystem.
        if (!file.exists()) {
            // Do your thing if the file appears to be non-existing.
            // Throw an exception, or send 404, or show default/warning page, or just ignore it.
            response.sendError(HttpServletResponse.SC_NOT_FOUND, "Podcast file does not exist: " + requestedFile);
            return;
        }
